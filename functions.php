<?php
function isimb_6310_link_css_js($hook)
{
   if(
      $hook != 'toplevel_page_isimb-6310-image-map-builder' && 
      $hook != 'wp-image-map-builder_page_isimb-6310-image-map-builder-import-export' && 
      $hook != 'image-map-builder_page_isimb-6310-image-map-builder-license' && 
      $hook != 'image-map-builder_page_isimb-6310-image-map-builder-use' && 
      $hook != 'image-map-builder_page_isimb-6310-image-map-builder-import-export' && 
      $hook != 'image-map-builder_page_isimb-6310-wpmart-plugins'
   ) {
   return;
}
   ...
   wp_enqueue_style('isimb-6310-color-style', plugins_url('assets/css/jquery.minicolors.css', __FILE__));
   wp_enqueue_style('isimb-6310-font-select-style', plugins_url('assets/css/fontselect.css', __FILE__));
   wp_enqueue_style('isimb-6310-font-awesome-5-0-13', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.9.0/css/all.min.css');
   wp_enqueue_style('isimb-6310-codemirror-style', 'https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.48.0/codemirror.min.css');
   wp_enqueue_style('isimb-6310-color-style', 'https://cdnjs.cloudflare.com/ajax/libs/jquery-minicolors/2.3.4/jquery.minicolors.min.css');
   wp_enqueue_style('isimb-6310-jquery-ui-css', "https://cdnjs.cloudflare.com/ajax/libs/jqueryui/1.12.1/jquery-ui.min.css");

   
   wp_enqueue_script('isimb-6310-font-select-js', plugins_url('assets/js/fontselect.js', __FILE__), array('jquery'));
   wp_enqueue_script('isimb-6310-jquery-ui-js', 'https://cdnjs.cloudflare.com/ajax/libs/jqueryui/1.12.1/jquery-ui.min.js', array('jquery'));
   wp_enqueue_script('isimb-6310-color-js', 'https://cdnjs.cloudflare.com/ajax/libs/jquery-minicolors/2.3.4/jquery.minicolors.min.js', array('jquery'));
   wp_enqueue_script('isimb-6310-codemirror-js', 'https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.48.0/codemirror.min.js', array('jquery'));
   wp_enqueue_script('isimb-6310-common', plugins_url('assets/js/isimb-6310-common.js', __FILE__), array('jquery'));
   wp_enqueue_script('isimb-6310-json-js', plugins_url('assets/js/json-data.js', __FILE__), array('jquery'));
   wp_enqueue_script('isimb-6310-admin-js', plugins_url('assets/js/isimb-6310-admin-script.js', __FILE__), array('jquery'));
   wp_enqueue_script('isimb-6310-modal-js', plugins_url('assets/js/isimb-6310-admin-modal.js', __FILE__), array('jquery'));
   wp_enqueue_script('isimb-6310-nested-point-js', plugins_url('assets/js/isimb-6310-nested-point.js', __FILE__), array('jquery'));
   wp_enqueue_script('isimb-6310-canvas-js', plugins_url('assets/js/jquery.canvas.js', __FILE__), array('jquery'));
}

function isimb_6310_replace($data) {
	if($data == '') return ''; 
...
function isimb_6310_get_user_roles(){
...
function isimb_6310_extract_data($data)
{
   $array_key = "";
   $array_value = "";
   $i = 0;
...
function isimb_6310_search_template($ids, $cssData, $output = 0){
   if(!isset($cssData['search_activation']) && $output) return;
	?>
	<div class="isimb-6310-search isimb-6310-search-<?php echo esc_attr($ids) ?>">
		<div class="isimb-6310-search-container isimb-6310-search-template-<?php echo esc_attr($ids) ?>">
			<input type="text" name="isimb-6310-search-box"  data-isimb-6310-template-id='<?php echo esc_attr($ids) ?>' class="isimb-6310-search-box" autocomplete="off" placeholder="<?php echo (isset($cssData['search_placeholder']) && $cssData['search_placeholder'] !== '') ? esc_attr($cssData['search_placeholder']) : '' ?>">
   ...
   function isimb_6310_builder_install()
   {
      global $wpdb;
   ...
   function isimb_6310_get_option($name) {
		global $wpdb;
   ...
   function isimb_6310_version_status() {
      global $wpdb;
      $isimb_6310_textarea_updated = isimb_6310_get_option('isimb_6310_textarea_updated');
      if(!$isimb_6310_textarea_updated){
         $wpdb->query("DELETE FROM {$wpdb->prefix}options where option_name='isimb_6310_textarea_updated'");
         $wpdb->query("INSERT INTO {$wpdb->prefix}options(option_name, option_value) VALUES ('isimb_6310_textarea_updated', '1')");
         $wpdb->query("ALTER TABLE {$wpdb->prefix}isimb_6310_style MODIFY css LONGTEXT");
      }

      $db_version = isimb_6310_get_option('isimb_6310_version_info');
      if(!$db_version){
         $wpdb->query("DELETE FROM {$wpdb->prefix}options where option_name='isimb_6310_version_info'");
            $wpdb->query("INSERT INTO {$wpdb->prefix}options(option_name, option_value) VALUES ('isimb_6310_version_info', '".isimb_6310_PLUGIN_CURRENT_VERSION."')");
      }
      else{
         $key = isimb_6310_get_option('isimb_6310_license_key');
         if($db_version != isimb_6310_PLUGIN_CURRENT_VERSION && $key){
            isimb_6310_check_license($key, true);
            $wpdb->query("UPDATE {$wpdb->prefix}options set 
                        option_value='". isimb_6310_PLUGIN_CURRENT_VERSION ."' 
                        where option_name = 'isimb_6310_version_info'");
         }	
      }
   } 
   
   function isimb_6310_check_license($key, $autoUpdate = false) {
      global $wpdb;
  
      // Use WordPress options API for retrieving/storing options
      $db_key = sanitize_text_field(get_option('isimb_6310_license_key'));
  
      if (!$db_key) {
          update_option('isimb_6310_license_key', sanitize_text_field($key));
      } elseif ($db_key !== $key) {
          update_option('isimb_6310_license_key', sanitize_text_field($key));
      }
      ...
      $api_params = array(
          ...
          'item_name'  => urlencode('Interactive SVG Image Map Builder'),
      ...
      if (is_wp_error($response)) {
          echo "<p class='isimb-6310-error-message'>" . esc_html($response->get_error_message()) . "</p>";
          return;
      }
  
      $license_data = json_decode(wp_remote_retrieve_body($response));
      if (!isset($license_data->success) || !$license_data->success) {
          $message = '<p class="isimb-6310-error-message">An error occurred, please try again.</p>';
          if (isset($license_data->error)) {
              switch ($license_data->error) {
                  case 'invalid_key':
                      $message = '<p class="isimb-6310-error-message">You have entered an invalid license key.</p>';
                      break;
                  case 'site_inactive':
                      $message = '<p class="isimb-6310-error-message">Your license is not active for this URL.</p>';
                      break;
              }
          }
          echo $message;
      ...
      if (is_wp_error($tmp_file)) {
          echo '<p class="isimb-6310-error-message">Error downloading the file.</p>';
          return;
      }
      ...
      if ($zip->open($tmp_file) === true) {
          ...
          if (!$autoUpdate) {
              echo "<p class='isimb-6310-success-message'>Congratulations! Your license activated successfully.</p>";
          }
      } else {
          echo '<p class="isimb-6310-error-message">There was an error unzipping the file.</p>';
      }
  }

   function isimb_6310_fa_icon_list($startTag, $endTag) {
      $iconArray = array (
               ...
               'fas fa-asterisk' => ['attention, details, reference, star', '\f069'],
   ...
   function isimb_6310_extract_item($ids)
   {
      global $wpdb;
      $item_table = $wpdb->prefix . 'isimb_6310_item';
      $results = array();
      if($ids) {
         $ids = explode(',', $ids);
         if($ids) {
...
function isimb_6310_display_icon($ids, $type, $icons, $hover_icons, $image, $hover_image) {
   echo "
      <div class='isimb-6310-template-".esc_attr($ids)."-icon-wrapper'>
         <div class='isimb-6310-template-".esc_attr($ids)."-icon'>";
         if($type == 1) {
            $hover_icons = $hover_icons ? $hover_icons : $icons;
            echo "
                  <i class='".esc_attr($icons)." isimb-6310-icon'></i>
                  <i class='".esc_attr($hover_icons)." isimb-6310-hover-icon'></i>
               ";
         } else{
            $hover_image = $hover_image ? $hover_image : $image;
            echo "
                  <img src='".esc_attr($image)."' class='isimb-6310-image'>
                  <img src='".esc_attr($hover_image)."' class='isimb-6310-hover-image'>
               ";
         }
   echo  "</div>
      </div>
...
function isimb_6310_display_read_more($ids, $target, $text, $url, $output = 1) {
   if(!strlen($text)) return;
   if(!$output) return;
   $target = $target ? "target='_blank'" : '';
   echo "
   <div class='isimb-6310-template-".esc_attr($ids)."-read-more'>
      <a href='{$url}' {$target}>{$text}</a>
   </div>
   ";
}

function isimb_6310_add_new_media($id, $results = [])
   {
      global $wpdb;
      wp_enqueue_media();
      $style_table = $wpdb->prefix . 'isimb_6310_style';
      $item_table = $wpdb->prefix . 'isimb_6310_item';
?>
      <div class="isimb_6310_add_media">
         <h6>Shortcode</h6>
         <div class="isimb_6310_add_media_body_shortcode">
            <input type="text" onclick="this.setSelectionRange(0, this.value.length)" value='[isimb_6310_builder id="<?php echo esc_attr($id); ?>"]' />
         </div>
      </div>
      <br />
      <div class="isimb_6310_add_media">
         <h6>Customize Items</h6>
         <div class="isimb_6310_add_media_body" id="isimb-6310-add-or-edit-items">
            <i class="fas fa-plus-circle isimb_6310_add_media_add_new_icon"></i>
            Add/Edit Items
         </div>
      </div>
      <br />
      <div class="isimb_6310_add_media">
         ...
         <div class="isimb_6310_add_media_body" id="isimb-6310-rearrange-items">
            <i class="fas fa-user-cog isimb_6310_add_media_add_new_icon"></i>
         </div>
      </div>
      <br />
      <div class="isimb_6310_add_media">
         <h6>How to Use</h6>
         <div class="isimb_6310_add_media_body">
            <a href="https://www.youtube.com/watch?v=2VZ6Qa6GmX0" target="_blank">
...
<?php
   ...
   function isimb_6310_description($ids, $description, $output = 1){
      if(!$description) return;
      if(!$output) return;
      echo "<div class='isimb-6310-template-".esc_attr($ids)."-description'>".esc_attr($description)."</div>";
   }

   function isimb_6310_export_full_map_builder_plugin() {
      if( !ini_get('allow_url_fopen') ) {
       ...
       $path = wp_upload_dir();
       $name =  '/' . $wpdb->prefix . 'interactive-image-map-builder-backup.txt';
       $fileName = $path['path'] . $name;
       
       $table_name = $wpdb->prefix . 'isimb_6310_style';  
       ...
       if (!empty($data)) {
          ...
          echo '<a href="'. $path['url'].$name .'" target="_blank" id="export-image-map-builder-plugin">Download</a>';
       } else {
          echo '<a href="#" id="export-image-map-builder-plugin">Download</a>';
       }
   }
   
   function isimb_6310_import_full_plugin($url) {
		if( !ini_get('allow_url_fopen') ) {
			echo "<p style='color: green; font-size: 14px;'>
			In your cPanel, default allow_url_fopen is not enable. You need to enable. Please contact with hosting providers or 	follow the below URL. <br />
			<a href='https://www.youtube.com/watch?v=tUW6CkZEW8k'>How to Enable Allow_url_fopen in cPanel</a>
   ...
   function isimb_6310_validate_url($url){
      if ($url != '' && substr($url, 0, 7) != "http://" && substr($url, 0, 8) != "https://"){
         $url = "http://{$url}";
      }
      return $url;
   }
   function isimb_6310_load_templates($js, $counter, $ids) {
      $html = "";
      $customCSS = "
      .isimb-6310-builder-box-{$ids} .isimb-6310-point-{$counter}  .isimb-6310-pin-main-img {           
         color: red;
         font-size: 15px;
      }    
      .isimb-6310-builder-box-{$ids} .isimb-6310-point-{$counter} .isimb-6310-pin-hover-img {           
         color: yellow;
         font-size: 15px;
      }   
      ";    
      if($js->viewMoodType == '2'){
         echo "
               <div class='isimb-6310-hover-content isimb-6310-hover-content-{$ids}-{$counter}'>
                  <div class='isimb-6310-hover-content-{$ids}-{$counter}-element'>
                     <div class='isimb-6310-template-modal-hover-content'>".esc_attr($js->selectModelContent)."</div>
                  </div>
               </div>";
         $customCSS .="
         .isimb-6310-hover-content-{$ids}-{$counter}-element .isimb-6310-template-modal-hover-content{
      ...
      if($js->viewMoodType == '1'){
         $className1 = "isimb-6310-hover-content";   
         $className2 = "isimb-6310-hover-content-{$ids}-{$counter}";
         $closeButton = 'isimb-6310-close-button isimb-6310-close-button-mobile';
      } else {
         $className1 = "isimb-6310-modal-content hasan-$js->viewMoodType";   
         $className2 = "isimb-6310-modal-content-{$ids}-{$counter}";   
         $modal_start = "<div class='isimb-6310-modal isimb-6310-popup-{$ids}-{$counter}'><div class='isimb-6310-modal-xl'>";
         $modal_end = "</div></div>";
         $modal_element = 'isimb-6310-modal-content-element ';
         $closeButton = 'isimb-6310-close-button';
      }  

      $linkURL = $js->linkURL ? isimb_6310_validate_url(esc_attr($js->linkURL)) : '';
      $link_start = '';
      $link_end = '';
      if($linkURL){
         if(!isset($js->linkingArea) || $js->linkingArea != 1 || $js->viewMoodType == '2'){
            if($js->linkText){
               $link_target = (isset($js->openNewTab) && $js->openNewTab ? " target='_blank'" : '');
               $link_start = "<a href='{$linkURL}'{$link_target}>";
               $link_end = "</a>";
            }
         } else if($js->viewMoodType == '1' && ($js->elementType != '3' || ($js->elementType == '2' && $js->selectedTemplate != '02'))){
            $link_target = (isset($js->openNewTab) && $js->openNewTab ? 1 : 0);
            $html .= "window.addEventListener('load', function(){jQuery('.isimb-6310-pol-{$ids}-{$counter}, polygon[data-nested-id=\'{$ids}-{$counter}\']').attr({'isimb-6310-direct-link':'{$linkURL}','isimb-6310-direct-link-target':'{$link_target}'});});";
         }
      }
      ...
      if($js->elementType == '1' || $js->elementType == '2'){        
         ...
         if($js->selectedTemplate == '01') {
            if($js->linkText){
               ...
               .{$className2}-element a, .{$className2}-element a:hover {
                  color: ".esc_attr($js->tempCommonFontColor).";
                  display: block;
               }
            ";   
                           ...
                           <div class='isimb-6310-tooltip isimb-6310-template-02'>      
                              <div class='isimb-6310-template-02-hover-content'>
                                 <div class='isimb-6310-template-02-content'>
                                    <div class='{$closeButton}'></div>
                                    ".isimb_6310_create_embedded_code($js->tem02EmbeddedLink)."                                     
                                 </div>
                              </div>
                           </div>
                        </div>
         ...
         // $customCSS .= "@media screen and (max-width: 767px) {.isimb-6310-hover-content-{$ids}-{$counter}{display: none}}";   
         }
            ...
            ";         
            $customCSS .= "
               .{$className2}-element .isimb-6310-template-03-tooltip-testimonial{
                  background-color: ".esc_attr($js->tempCommonBgColor).";
               }
               .{$className2}-element .isimb-6310-template-03-tooltip-testimonial .isimb-6310-template-03-tooltip-testimonial-title{
                  font-size: ".esc_attr($js->tempCommonFontSize)."px;
                  line-height: ".esc_attr($js->tempCommonFontSize + 6)."px;
                  color: ".esc_attr($js->tempCommonFontColor).";
                  padding-bottom:5px;
               ...
               .{$className2}-element .isimb-6310-template-03-tooltip-testimonial .isimb-6310-template-03-tooltip-description{
                  color: ".esc_attr($js->openDesFontColor).";
                  font-size:".esc_attr($js->openDesFontSize)."px;
                  line-height:".esc_attr($js->openDesFontSize + 6)."px;
               }
               .{$className2}-element .isimb-6310-template-03-tooltip-testimonial .isimb-6310-template-03-tooltip-testimonial-title a{
                  color: ".esc_attr($js->tempCommonFontColor).";
                  display: block;
               }
               .{$className2}-element .isimb-6310-template-03-tooltip-testimonial .isimb-6310-template-03-tooltip-description a{
                  color: ".esc_attr($js->openDesFontColor).";
                  display: block;
               }
            ";
                           ...
                           {$link_start}
                              <div class='isimb-6310-template-04-tooltip-testimonial'>
                                 <div class='isimb-6310-template-04-tooltip-testimonial-content'>
                                    ...
                                    <div class='isimb-6310-template-04-tooltip-title'>".isimb_6310_replace(esc_attr($js->linkText))."</div>
                                    <div class='isimb-6310-template-04-tooltip-description'>
                                    ".isimb_6310_replace(esc_attr($js->openDescription))."
                                    </div>        
                                 </div>
                              </div>
                           {$link_end}
            ...
            $customCSS .= "
               .{$className2}-element .isimb-6310-template-04-tooltip-testimonial{
                  background: ".esc_attr($js->tempCommonBgColor).";     
                  text-decoration: none !important;   
               }  
               .{$className2}-element .isimb-6310-template-04-tooltip-title{        
                  color: ".esc_attr($js->tempCommonFontColor).";
                  font-size:".esc_attr($js->tempCommonFontSize)."px;
                  line-height:".esc_attr($js->tempCommonFontSize + 6)."px;
                  text-decoration: none !important; ;
               ...
               .{$className2}-element .isimb-6310-template-04-tooltip-description{        
                  color: ".esc_attr($js->openDesFontColor).";
                  font-size:".esc_attr($js->openDesFontSize)."px;
                  line-height:".esc_attr($js->openDesFontSize + 6)."px;
                  text-decoration:none !important;
               ...
               .{$className2}-element .isimb-6310-template-04-tooltip-title a{        
                  color: ".esc_attr($js->tempCommonFontColor).";
                  display: block
               }  
               .{$className2}-element .isimb-6310-template-04-tooltip-description a{        
                  color: ".esc_attr($js->openDesFontColor).";
                  display: block;
               }       
            ";
            ...
            echo "
                           ...
                           <div class='isimb-6310-template-05-hover-content'>
                              ...
                              <div class='isimb-6310-template-05-tooltip-testimonial'>
                                 <div class='isimb-6310-template-05-tooltip-testimonial-content'>
                                    ...
                                    <div class='isimb-6310-template-05-tooltip-title'>".isimb_6310_replace(esc_attr($js->linkText))."</div>
                                 </div>
                  ...
                  ";
            $customCSS .= "
            .{$className2}-element .isimb-6310-template-05-hover-content{
               background: ".esc_attr($js->tempCommonBgColor).";        
            }  
            .{$className2}-element .isimb-6310-template-05-tooltip-testimonial-content .isimb-6310-template-05-tooltip-title {        
               color: ".esc_attr($js->tempCommonFontColor).";
               font-size:".esc_attr($js->tempCommonFontSize)."px;
               line-height:".esc_attr($js->tempCommonFontSize + 6)."px;
            }    

            .{$className2}-element .isimb-6310-template-05-tooltip-description {
               color: ".esc_attr($js->openDesFontColor).";
               font-size:".esc_attr($js->openDesFontSize)."px;
               line-height:".esc_attr($js->openDesFontSize + 6)."px;
            }     

            .{$className2}-element .isimb-6310-template-05-tooltip-testimonial-content .isimb-6310-template-05-tooltip-title a{        
               color: ".esc_attr($js->tempCommonFontColor).";
               display: block;
            }

            .{$className2}-element .isimb-6310-template-05-tooltip-description a{
               color: ".esc_attr($js->openDesFontColor).";
               display: block;
            }    
         ";
   ...
   if($modal_start){
      $customCSS .= "@media screen and (max-width: 767px) {.isimb-6310-popup-{$ids}-{$counter}{display: none;}}";
   }
  

   wp_register_style("isimb-6310-custom-" . esc_attr($ids) . "-" . esc_attr($counter) . "-css", "");
   wp_enqueue_style("isimb-6310-custom-" . esc_attr($ids) . "-" . esc_attr($counter) . "-css");
   wp_add_inline_style("isimb-6310-custom-" . esc_attr($ids) . "-" . esc_attr($counter) . "-css", $customCSS);

   wp_register_script("isimb-6310-template-" . esc_attr($counter) . "-js", "");
   wp_enqueue_script("isimb-6310-template-" . esc_attr($counter) . "-js");
   wp_add_inline_script("isimb-6310-template-" . esc_attr($counter) . "-js", $html); 
   }
